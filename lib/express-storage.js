...
exports.storage = (function() {
  var config=require('../../config').config,//got some problems with my .gitignore
    url=require('url'),
    redisClient=require('redis').createClient(config.redisPort, config.redisHost);
  ...
  function checkToken(userAddress, token, category, method, callback) {
    if (category == 'public' && method == 'GET') {
      ...
      console.log('looking for "'+category+'" in key "token:'+userAddress);

      redisClient.get('token:'+userAddress, function(err, tokensStr) {
        var tokens,scopes;
        try {
        ...
        if(!tokens[token][scope] || typeof(tokens[token][scope] != 'string')) {
          console.log('DATA CORRUPTION: no scope');
          callback(false);
          return;
        }
      ...
      redisClient.get('tokens:'+details.userAddress, function(err, data) {
        if(err) {
          ...
          for(var token in tokens) {
            if(tokens[token].app && tokens[token].app==details.app && tokens[token].scope && tokens[scope]=details.categories.join(',')) {
              console.log('exists');
              callback(null, details.redirectUriPrefix+tokens[details.app][details.categories.join(',')]);
              return;
            }
          }
          ...
          if(tokens[newToken]) {
            ...
            console.log(tokens);
            redisClient.set('token:'+details.userAddress, JSON.stringify(tokens), function(err2) {
              if(err2) {
                console.log(err2);
                callback(err2);
              } else {
                console.log(details.redirectUriPrefix+newToken);
